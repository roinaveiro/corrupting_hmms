...
df1 <- read_csv(paste0("results/ner_path_attraction/", fname1, ".csv") )
df1 <- df1 %>% mutate(exp_util =  exp_util - 30*(w1+w2) - 1)
df2 <- read_csv(paste0("results/ner_path_attraction/", fname2, ".csv") )
df2 <- df2 %>% mutate(exp_util =  exp_util - 30*(w1+w2) - 1)
...
df_sum1 <- df1 %>% select(-c(original_phrase, attacked_phrase)) %>% 
  ...
  summarise("w1" = mean(w1),
            ...
            "std_attacc" = sd(attacked_acc)) %>% mutate(comb = "1")
...
df_sum2 <- df2 %>% select(-c(original_phrase, attacked_phrase)) %>% 
  ...
  summarise("w1" = mean(w1),
            ...
            "std_originalacc" = sd(original_acc),
            "mean_attacc" = mean(attacked_acc),
            "std_attacc" = sd(attacked_acc)) %>% mutate(comb = "2")
...
p <- df %>% ggplot(aes(x = seconds, y = mean_EU)) + 
  ...
  theme_minimal() +
  #labs(title = "Attack performance. Path Attraction.", subtitle =subt, x = 
  #       "Running time (seconds)", y = "Expected Utility")  +
  theme(text = element_text(size=30)) + theme(panel.margin = unit(2, "lines"))

png( paste0("path_attraction_EU", ".png"), width = width , height = height,
...
if(FALSE){
  ...
  png( paste0(fname, ".png"), width = width , height = height, units = "px", 
       type = "quartz", pointsize = 15, res = 800)
  print(p)
  dev.off()
}
