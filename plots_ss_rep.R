...
df1 <- read_csv(paste0("results/ner_ss_repulsion/", fname1, ".csv") )
df1 <- df1 %>% mutate(exp_util =  exp_util - w1 - 30*(w2) - 1)
df2 <- read_csv(paste0("results/ner_ss_repulsion/", fname2, ".csv") )
df2 <- df2 %>% mutate(exp_util =  exp_util - w1 - 30*(w2) - 1)
...
df_sum1 <- df1 %>% select(-c(original_phrase, attacked_phrase)) %>% 
  ...
  summarise("w1" = mean(w1),
            ...
            "std_EU" = sd(exp_util),
            "mean_clean" = mean(p_clean),
            "std_clean" = sd(p_clean),
            "mean_att" = mean(p_att),
            "std_att" = sd(p_att),
...
p <- df %>% ggplot(aes(x = seconds, y = mean_EU)) + 
  ...
  theme_minimal() +
  labs(title = "Attack performance. State Repulsion", subtitle =subt, x = 
         "Running time (seconds)", y = "Expected Utility")  +
  theme(text = element_text(size=20)) + theme(panel.margin = unit(2, "lines"))

png( paste0("dist_state_rep_EU", ".png"), width = width , height = height,
...
if(FALSE){
  ...
  df_pivot <- df_sum %>%  pivot_longer(
    ...
    mutate(name = case_when(
      ...
      name == "clean" ~ "State Probability Clean Data",
      name == "att" ~ "State Probability Corrupted Data",
      TRUE ~ name
    ))
  ...
  p <- df_pivot %>% ggplot(aes(x = seconds, y = mean)) + 
    ...
    labs(title = "Attack performance metrics. Path Attraction.", subtitle =subt, x = 
