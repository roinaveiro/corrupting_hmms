...
data %>% filter(time %% 10 == 0) %>% 
  ...
  filter( (combination == "A" & solver %in% c("APS_1", "APS_500", "RME", "SA_D", "MCTS_B") ) |
          (combination == "B" & solver %in% c("APS_1", "APS_500", "RME", "SA_D", "MCTS_B") ) |
          (combination == "C" & solver %in% c("APS_1", "APS_500", "RME", "SA_D", "MCTS_A") ) |
          (combination == "D" & solver %in% c("APS_1", "APS_500", "RME", "SA_D", "MCTS_A") ) 
          ) %>%
...
ggsave(filename = "experiment_plots_r/exp_2_pd.png", 
       device = "png", 
