...
data %>% filter(time %% 10 == 0) %>% 
  ...
  combination = case_when(
        ...
        rho == 0.75 & k == 100 ~ "D"
             ...
             `|Q|` == QQ, `|X|` == XX, `|T|` == TT) %>% 
  filter( (combination == "A" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_E") ) |
          (combination == "B" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_E") ) |
          (combination == "C" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_E") ) |
          (combination == "D" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_E") ) 
          ) %>%
...
ggsave(filename = "experiment_plots_r/exp_2_rep.png", 
       device = "png", 
