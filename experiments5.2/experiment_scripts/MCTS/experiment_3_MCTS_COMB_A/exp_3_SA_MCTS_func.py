...
def create_attacker_obj_exp3(att_st, comb_num, params_d_):
    
    par_d  = params_d_[att_st]
    mat = params_d_['mat'][str(comb_num)]
    
    if att_st == 'atr':
        att = ss_attacker(mat['prior'],            
                     mat['trans'], 
                     mat['emiss'], 
                     mat['rho'],
                     mat['X'], 
                     w1 = par_d['w1'],
                     w2 = par_d['w2'] , 
                     t = par_d[str(comb_num)]['t'],
                     state = par_d[str(comb_num)]['state'],
                     c = par_d['c'],
                     k_value= params_d_['k'])
        ...
        att = dec_attacker(mat['prior'],            
                         ...
                         w1 = par_d['w1'],
                         w2 = par_d['w2'] , 
                          seq = par_d[str(comb_num)]['seq'], 
                         k_value= params_d_['k'])
    
    return att

def solver_attacker_problem_cum_RS_single_iter_exp3(att_st, 
                                               comb_num,
                                               solver_str, 
                                               params_d_,
                                               idx):

    attacker = create_attacker_obj_exp3(att_st, comb_num, params_d_)
    ...
    if save_dict['save_ite_b'] == True:        
        ...
        res_df.to_json(save_dict['save_ite_d_'+ solver_str]  + file_str +'.json', orient = 'split')
        res_df.to_csv(save_dict['save_ite_d_'+ solver_str]  + file_str +'.csv', index = False)
        
    return res_df

def parallel_solver_attacker_problem_cum_RS_single_iter_exp3(att_st_, 
                                               comb_num_,
                                               solver_str_, 
                                               params_d__):
    ...
    res =  Parallel(n_jobs=num_jobs)(delayed(solver_attacker_problem_cum_RS_single_iter_exp3)(att_st = att_st_,
                                                       comb_num = comb_num_,
    ...
    if save_dict['save_gb_b'] == True:
        attacker = create_attacker_obj_exp3(att_st_, comb_num_, params_d__)
        set_up_str =  save_dict['id_str'] +'_'+ solver_str_ +'_'+att_st_
        hmm_str = '_Q_' + str(attacker.n_components) + '_X_'+ str(attacker.n_obs) + '_T_' + str(attacker.T)
        unc_str = '_rho_' + str(attacker.rho_probs[0]) + '_k_' + str(attacker.k_value)
        file_str = set_up_str + hmm_str + unc_str 
        res_df.to_json(save_dict['save_gb_d_'+ solver_str_]  + file_str +'.json', orient = 'split')
        res_df.to_csv(save_dict['save_gb_d_'+ solver_str_]  + file_str +'.csv', index = False)
