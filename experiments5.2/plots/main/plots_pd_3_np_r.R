...
data %>% filter(time %% 10 == 0) %>% 
  ...
  combination = case_when(
        ...
        `|Q|` == 30 & `|X|` == 10 & `|T|` == 10 ~ "D"
             ...
             rho == rho_, k == k_) %>% 
  filter( (combination == "A" & solver %in% c("APS_1", "APS_500", "RME", "SA_D", "MCTS_B") ) |
          (combination == "B" & solver %in% c("APS_1", "APS_500", "RME", "SA_A", "MCTS_A") ) |
          (combination == "C" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_A") ) |
          (combination == "D" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_A") ) 
          ) %>%
...
ggsave(filename = "experiment_plots_r/exp_3_pd.png", 
       device = "png", 
