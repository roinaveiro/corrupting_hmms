...
data %>% filter(time %% 10 == 0) %>% 
  ...
  filter( (combination == "A" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_B") ) |
          (combination == "B" & solver %in% c("APS_1", "APS_500", "RME", "SA_E", "MCTS_E") ) |
          (combination == "C" & solver %in% c("APS_1", "APS_500", "RME", "SA_B", "MCTS_B") ) |
          (combination == "D" & solver %in% c("APS_1", "APS_500", "RME", "SA_E", "MCTS_A") ) 
          ) %>%
...
ggsave(filename = "experiment_plots_r/exp_3_dd.png", 
       device = "png", 
